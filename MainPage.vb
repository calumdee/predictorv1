...
Public Class MainPage
    ...
    Private Sub createInputBoxes(tab As Integer, leagueNumber As Integer, location As Point, inputNumber As Integer)
        ...
        ElseIf tab = 3 Then
            createButton("(2," + (c + 1).ToString + ")", "Right", csize, tab, tlocation)
            Dim button As Button = TC1.TabPages(tab).Controls("btn(1," + (c + 1).ToString + ")")
            AddHandler button.Click, AddressOf Me.method2
            button = New Button
            button = TC1.TabPages(tab).Controls("btn(2," + (c + 1).ToString + ")")
            AddHandler button.Click, AddressOf Me.method1
        End If
        Me.Size = New Size(Me.Size.Width, startLocation.Y + 70 + csize.Height)
        Me.Controls("TabControl1").Size = New Size(Me.Size.Width, startLocation.Y + 70 + csize.Height)
    End Sub
    ...
    Private Sub Predict_Click(sender As System.Object, e As System.EventArgs)
        ...
        Dim counter, pot, potentialLookBack, maxAll, setting As Integer
        ...
        If text = "1" Then
            currentMethod(pot, potentialLookBack, maxAll)
            chosenweek = x
            calcSett(maxAll)
            MessageBox.Show(position & "," & tform & "," & lform & "," & homeAdv)
        ElseIf text = "2" Then
        ...
        createTable()
        chosenweek = x
        counter = (TabControl1.TabPages(tab).Controls.Count - 9) / counter
        ...
        For l As Integer = 3 To counter + 2
            ...
            If Not (hcombo.Text = "" Or acombo.Text = "") Then
                If text = "1" Then
                    TabControl1.TabPages(tab).Controls("lbl(3," + l.ToString + ")").Text = exactScore(hcombo.Text, acombo.Text, predict(hcombo.Text, acombo.Text)).ToString + " , " + Math.Round(holder, 2).ToString
                ElseIf text = "2" Then
                    hpred = predict(hcombo.Text, acombo.Text)
                    'MessageBox.Show(w1(0) & "," & mTform & "," & mLform & "," & mPos & "," & mRTform & "," & mRLform & "," & mXG & "," & xhomeS & "," & xawayC)
                    hpred = w1(0) + w1(1) * htform + w1(2) * atform + w1(3) * mPos + w1(4) * hhform + w1(5) * aaform + w1(6) * 0 + w1(7) * xhomeS + w1(8) * xawayC

                    apred = w2(0) + w2(1) * htform + w2(2) * atform + w2(3) * mPos + w2(4) * hhform + w2(5) * aaform + w2(6) * 0 + w2(7) * xawayS + w2(8) * xhomeC
    ...
    Private Function exactScore(hName As String, aName As String, prediction As Decimal) As Decimal
        Dim potential, previous As Decimal
        Dim c1, c2, best(2), rounded As Integer
        Dim cleague As League = chosenLeague
        holder = prediction
                ...
                If (tmatch.gethTeam = hName Or tmatch.getaTeam = hName) And (c1 > 0 Or h1 > 0) Then
                    If tmatch.gethTeam = hName Then
                        ...
                        If h1 > 0 Then
                            ...
                            home(0) = tmatch.gethScore
                            home(1) = tmatch.getaScore
                            relLform += a
                ...
                If (tmatch.getaTeam = aName Or tmatch.gethTeam = aName) And (c2 > 0 Or a1 > 0) Then
                    ...
                    Else
                        ...
                        If a1 > 0 Then
                            ...
                            away(0) = tmatch.getaScore
                            away(1) = tmatch.gethScore
                            relLform -= a
    ...
    Private Sub getLeagues()
        ...
        Dim reader As StreamReader = My.Computer.FileSystem.OpenTextFileReader("C:\Users\calum\Documents\Predictor\Leagues.txt")
    ...
    Private Sub getGames(league As String)
        Dim reader As StreamReader = My.Computer.FileSystem.OpenTextFileReader("C:\Users\calum\Documents\Predictor\" & league & ".txt")
    ...
    Private Sub setGames()
        ...
        Dim writer As StreamWriter = My.Computer.FileSystem.OpenTextFileWriter("C:\Users\calum\Documents\Predictor\" & league & ".txt", False)
        Dim c As Integer
        Dim temp As Match
        For i As Integer = 0 To weeklyScores.Count - 1
            ...
            For j As Integer = 0 To c - 1
                ...
                writer.WriteLine(temp.gethTeam + "," + (temp.gethScore).ToString + "," + (temp.getaScore).ToString + "," + temp.getaTeam + "," + Math.Round(temp.getaprediction, 3).ToString + "," + Math.Round(temp.gethxG, 2).ToString + "," + Math.Round(temp.getaxG, 2).ToString)
            Next
            writer.WriteLine("n")
        Next
    ...
    End Sub
    Private Sub method1(sender As System.Object, e As System.EventArgs)
        Dim TabControl1 As TabControl = Me.Controls("TabControl1")
        Dim btn As Button = sender
        Dim tab As Integer = sender.tag
        Dim pot, potentialLookBack, maxAll As Decimal
        currentMethod(pot, potentialLookBack, maxAll)
        TabControl1.TabPages(TAB).Controls("lbl(3," + 3.ToString + ")").Text = potentialLookBack.ToString + " - " + pot.ToString
    End Sub
    Private Sub currentMethod(ByRef pot As Integer, ByRef potentialLookBack As Integer, ByRef maxall As Integer)
        Dim x As Integer
        Dim hName, aName As String
        Dim tmatch As Match
        Dim result, prediction As Decimal
        x = chosenweek - 1
        Dim a1, a2, b1, b2 As Decimal
        Dim setting(11), cumtotal(11) As Integer
        ...
        For i As Integer = 0 To x
            ...
            For h As Integer = 1 To 10
                ...
                createTable()
                chosenweek = i
                If h = maxAll And i = 1 Then
                    'MessageBox.Show(maxAll)
                    'MessageBox.Show(homeAdv)
                    'MessageBox.Show(tform)
                    'MessageBox.Show(lform)
                ...
                For j As Integer = 0 To weeklyScores(i).Count - 1
                    ...
                    prediction = exactScore(hName, aName, predict(hName, aName))
                    If h = maxAll And i = 1 Then
                        'MessageBox.Show(hName & " , " & checker)
                    End If

                    weeklyScores(i)(j).setprediction(prediction)
                    If h = maxAll Then
                        weeklyScores(i)(j).setaprediction(holder)
                    End If
                    ...
                    End If
                    If h = 10 Then
                        'MessageBox.Show(weeklyScores(i)(j).gethTeam & weeklyScores(i)(j).getaTeam & prediction.ToString)
                    End If
                Next
                ...
                Next
            Next
            If setting.Max / weeklyScores(i).Count < 0.6 Then
                'MessageBox.Show(i + 1)
            End If
            potentialLookBack += setting(maxAll)
            For v As Integer = 0 To 10
                cumtotal(v) += setting(v)
            Next
            For v As Integer = 0 To 10
                If cumtotal(v) = cumtotal.Max Then
                    maxAll = v
                    'MessageBox.Show(setting(v) & " " & v)
                End If
            Next
            pot = pot + setting.Max
        ...
        Next
        MessageBox.Show(maxAll)
        MessageBox.Show(cumtotal(1))
    End Sub
    Private Sub justCalc1()
        calcSett(5)
    End Sub
    Private Sub justCalc(sender As System.Object, e As System.EventArgs)
        calcSett(5)
    End Sub
    Private Sub calcSett(reduce As Integer)
        Dim tmatch As Match
        Dim hName, aName, array() As String
        Dim noOfTeam, Predictor(11), counter, bcount As Integer
        Dim prediction, result As Decimal
        ...
        End If
        If chosenweek > weeklyScores.Count Then
            counter = weeklyScores.Count
        Else
            counter = chosenweek
        End If
        If counter < reduce Then
            bcount = 0
        Else
            bcount = counter - reduce
        End If
        For g As Decimal = bcount To counter - 1
            chosenweek = g
            createTable()
            chosenweek = g
            For h As Integer = 0 To weeklyScores(g).Count - 1
                tmatch = New Match
                tmatch = weeklyScores(g)(h)
                hName = tmatch.gethTeam
                aName = tmatch.getaTeam
                result = tmatch.gethScore + tmatch.getaScore / 10
                For i As Decimal = 0 To 0.5 Step 0.05
                    homeAdv = i
                    For j As Decimal = 0.1 To 0.8 Step 0.1
                        tform = j
                        For k As Decimal = 0.1 To 0.9 - j Step 0.1
                            lform = k
                            For l As Decimal = 0 To 0 Step 0.2
                                position = 1 - j - k
                                prediction = exactScore(hName, aName, predict(hName, aName))
                                If prediction = result Then
                                    test.setStorage(i, j, k, position, 3, 3)
                                ElseIf (((convScoreDiff(prediction) > 0) And (convScoreDiff(result) > 0)) Or ((convScoreDiff(prediction) < 0) And (convScoreDiff(result) < 0)) Or ((convScoreDiff(prediction) = 0) And (convScoreDiff(result) = 0))) Then
                                    test.setStorage(i, j, k, position, 1, 1)
                                End If
                            Next
            ...
            Next
        Next
        ReDim array(noOfTeam / 2)
        array = test.returnBest(noOfTeam / 2)
        Dim x As Integer = 0
        Dim temp() As String
        Dim updated As Boolean = False
        While x < array.Length And Not (updated)
            temp = array(x).Split(New Char() {","c})
    ...
    Private Sub ml(ByRef setting As Integer, ByRef w1() As Decimal, ByRef w2() As Decimal)
        ...
        Dim hName, aName As String
        Dim tmatch As Match
        Dim result, prediction, hprediction, aprediction As Decimal
        x = chosenweek - 1
        Dim a1, a2, b1, b2 As Decimal
        Dim X1_test, X2_test As New List(Of List(Of Decimal))
        Dim y1, y2, new_x As New List(Of Decimal)
